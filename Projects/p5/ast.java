...
class ExpListNode extends ASTnode {
    ...
    public void typeCheck(List<Type> types) {
        for (int i = 0; i < myExps.size(); i++) {
            // Todo: Check for out of bounds? => Not needed, checked when called
...
class PostIncStmtNode extends StmtNode {
    ...
    /**
     * We must check if we are incrementing an integer.
     */
    public void typeCheck(Type returnType) {
...
class PostDecStmtNode extends StmtNode {
    ...
    /**
     * We must check if we are decrementing an integer.
     */
    public void typeCheck(Type returnType) {
        Type type = myExp.typeCheck();
...
class IfStmtNode extends StmtNode {
    ...
    public void typeCheck(Type returnType) {
        ...
        // myExp passed type checking?
        if (type.isErrorType() || type.isBoolType()) {
            // If so, check statements within it...
            myStmtList.typeCheck(returnType);
            return;
        }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void typeCheck(Type returnType) {
        ...
        // Check both anyways, even if error above
...
class ReturnStmtNode extends StmtNode {
    ...
    public void typeCheck(Type returnType) {
        ...
        // We expect this function to be a void type, since there is an expression
        // being returned...
        if (returnType.isVoidType()) {
        ...
        // Todo: Ensure this logic is right?
        // It must be equal type, or an error type to pass...
        if (!type.equals(returnType) && !type.isErrorType()) {
...
abstract class ExpNode extends ASTnode {
    ...
    /**
     * Abstracted method to grab the line num of the start of the expression
     */
    public abstract int lineNum();

    /**
     * Abstracted method to grab the character num of the start of the expression
     */
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (!leftType.equals(rightType) && !leftType.isErrorType() && !rightType.isErrorType()) {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (myId.sym() == null) {
            System.err.println("[debug] null symbol for myId.sym()?");
            System.exit(-1);
        }

        // type.isFnType() => it is a FnSym
    ...
    // 2 kids
    private IdNode myId;
    private ExpListNode myExpList;  // possibly null (i don't think so??)
}
...
abstract class UnaryExpNode extends ExpNode {
    ...
    /**
     * Convenince method for unary nodes.
     */
    public int lineNum() {
...
class EqualsNode extends BinaryExpNode {
    ...
    public Type typeCheck() {
        ...
        int line = myExp1.lineNum();
        int chr = myExp1.charNum();
        ...
        boolean same = type1.equals(type2);

        // Now the rest of the checks we can assume type1.equals(type2).

        if (type1.isVoidType() && same) {
            ErrMsg.fatal(line, chr, "Equality operator applied to void functions");
        }
        else if (type1.isFnType() && same) {
            ErrMsg.fatal(line, chr, "Equality operator applied to functions");
        }
        else if (type1.isStructType() && same) {
            ErrMsg.fatal(line, chr, "Equality operator applied to struct variables");
        }
        else if (type1.isStructDefType() && same) {
            ErrMsg.fatal(line, chr, "Equality operator applied to struct names");
        }
        else {
            // equality operators are bools
            result = new BoolType();
        }
