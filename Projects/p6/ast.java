...
class ProgramNode extends ASTnode {
    ...
    public void nameAnalysis() {
        ...
        // print the error message: "No main function". Use 0,0 as the line and character numbers
...
class FnBodyNode extends ASTnode {
    ...
    public void nameAnalysis(SymTable table) {
        throw new RuntimeException("not analyzed with offset");
    }
     ...
     */
    public void nameAnalysis(SymTable symTab, Integer offset) {
        myDeclList.nameAnalysis(symTab);
    ...
    public void codeGen(String branchLabel) {
        myStmtList.codeGen(branchLabel);
    }
    ...
    public SemSym nameAnalysis(SymTable symTab, SymTable globalTab) {
        ...
        if (!badDecl) {  // insert into symbol table
            try {
                ...
                // Any scope level greater than 1 implies within a function
                if (symTab.scopeLevel() == 1) {
                    sym.setGlobal(true);
                }
...
class FnDeclNode extends DeclNode {
    ...
    public SemSym nameAnalysis(SymTable symTab) {
        ...
        if (sym != null) {
            // TODO: Check this
            paramOffset = 4 * myFormalsList.length();
            sym.setOffset(paramOffset);

            sym.addFormals(typeList);
        }

        int offset = 8 + paramOffset;
        
        myBody.nameAnalysis(symTab, 4 * myBody.getDeclList().size() + offset); // process the function body
        
        for (DeclNode node : myBody.getDeclList()) {
            if (node instanceof VarDeclNode) {
                VarDeclNode var = (VarDeclNode) node;
                var.getId().sym().setOffset(offset);
                offset += 4;
            }
        }

        localOffset = offset - 8 - paramOffset;
    ...
    public void codeGen() {
        ...
        // body
        String exit = "EXIT_" + myId.name();
        myBody.codeGen(exit);

        // jump to exit after body
...
class PostIncStmtNode extends StmtNode {
    ...
    public void codeGen(String label) {
        // other types wouldn't make sense
...
class WriteStmtNode extends StmtNode {
    ...
    public void codeGen(String label) {
        ...
        int param = myExp instanceof StringLitNode ? 4 : 1;

        Codegen.generate("li", Codegen.V0, param);
        Codegen.generateWithComment("syscall", "write var");
    }
...
class IfStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        symTab.addScope();
        myDeclList.nameAnalysis(symTab);
        myStmtList.nameAnalysis(symTab, 4 * myDeclList.getList().size() + offset);
        for (DeclNode node : myDeclList.getList()) {
            if (node instanceof VarDeclNode) {
                VarDeclNode var = (VarDeclNode) node;
                var.getId().sym().setOffset(offset + this.offset);
                this.offset += 4;
            }
        }
        try {
...
class IfElseStmtNode extends StmtNode {
    ...
    public void nameAnalysis(SymTable symTab, int offset) {
        ...
        for (DeclNode node : myThenDeclList.getList()) {
            if (node instanceof VarDeclNode) {
                VarDeclNode var = (VarDeclNode) node;
                var.getId().sym().setOffset(offsetIf + offset);
                offsetIf += 4;
            }
        }
        try {
        ...
        for (DeclNode node : myElseDeclList.getList()) {
            if (node instanceof VarDeclNode) {
                VarDeclNode var = (VarDeclNode) node;
                var.getId().sym().setOffset(offsetElse + offset);
                offsetIf += 4;
            }
        }
        try {
...
class StringLitNode extends ExpNode {
    // must store strings so that comparing them works
    private static final Map<String, String> STRS = new HashMap<>();
    ...
    public void codeGen() {
        // may be null
        String label = STRS.get(myStrVal);

        if (label == null) {
            label = Codegen.nextLabel();
            STRS.put(myStrVal, label);
...
class EqualsNode extends EqualityExpNode {
    ...
    public void codeGen() {
        String jumpLabel = Codegen.nextLabel();
        ...
        Codegen.generate("b", exit);

        Codegen.genLabel(jumpLabel);
